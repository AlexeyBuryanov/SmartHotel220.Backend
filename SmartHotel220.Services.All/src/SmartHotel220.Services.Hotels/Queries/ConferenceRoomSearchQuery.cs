...
{
    /// <summary>
    /// Запросы связанные с поиском конференц-румы
    /// </summary>
    public class ConferenceRoomSearchQuery
    {
        ...
        /// <summary>
        /// Получить конференц-залы по фильтру
        /// </summary>
        public async Task<IEnumerable<ConferenceRoomSearchResult>> Get(ConferenceRoomSearchFilter filter)
        {
            ...
            result.ForEach(crs => crs.Picture = crs.NumPhotos > 0 
                                                    ? $"picconf/{crs.Id}_1.png" 
                                                    : "");

            return result;
        } // Get
    } // ConferenceRoomSearchQuery
